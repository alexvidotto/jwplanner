...
export const transformWeekToFrontend = (week: any) => {
  ...
  week.designacoes?.forEach((d: any) => {
    ...
    const part = {
      ...
      assistantStatus: (!template.requerLeitor && d.ajudanteId) ? (d.statusAjudante || 'PENDENTE') : 'PENDENTE',
      readerStatus: (template.requerLeitor && d.ajudanteId) ? (d.statusAjudante || 'PENDENTE') : 'PENDENTE',
