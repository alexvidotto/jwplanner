...
import { Outlet, useSearchParams, useBlocker } from 'react-router-dom';
...
export const MainLayout = () => {
  ...
  useEffect(() => {
    ...
    // Reset dirty state when week loads/changes from API
    setIsDirty(false);
  ...
  const handleNavigateWeek = (direction: number) => {
    ...
    // setActiveWeek(null); // Let the effect handle it to avoid flicker or inconsistent state
  };
        ...
        weekToSave.sections.forEach((s: any) => s.parts.forEach((p: any) => {
          ...
          if (match) {
            ...
            updates.push({
              ...
              assistantStatus: p.assistantStatus || p.assistantStatus || p.readerStatus || 'PENDENTE',
      ...
      <ConfirmModal
        ...
        onConfirm={() => {
          if (pendingNavigation) {
            setIsDirty(false); // Force clear dirty
            pendingNavigation();
            setPendingNavigation(null);
          }
        }}
